OC.L10N.register(
    "files_archive",
    {
    "The user folder for user \"%s\" could not be opened." : "La carpeta de usuario para el usuario \"%s\" no pudo abrirse.",
    "The archive file \"%s\" could not be found on the server." : "El archivo de contenedor \"%s\" no se ha encontrado en el servidor.",
    "Unable to open the archive file \"%s\": %s." : "No fue posible abrir el archivo de contenedor \"%s\": %s.",
    "Error: %s" : "Error: %s",
    "Unable to open the archive file \"%s\"." : "No fue posible abrir el archivo de contenedor \"%s\".",
    "The archive-file \"%1$s\" appears to be a zip-bomb: uncompressed size %2$s > admin limit %3$s." : "El archivo-contenedor \"%1$s\" parece ser un zip-bomb: El tamaño sin comprimir %2$s > límite administrativo %3$s.",
    "The archive-file \"%1$s\" is too large: uncompressed size %2$s > user limit %3$s." : "El archivo-contenedor \"%1$s\" es demasiado grande: El tamaño sin comprimir %2$s > límite de usuario %3$s.",
    "Unable to open the target parent-folder \"%s\"." : "No fue posible abrir la carpeta superior \"%s\".",
    "The target-folder \"%s\" already exists and auto-rename is not enabled." : "La carpeta destino \"%s\" ya existe y el renombrado automático no está habilitado.",
    "Unable to extract \"%1$s\" to \"%2$s\": \"%3$s\"." : "No fue posible extraer \"%1$s\" a \"%2$s\": \"%3$s\".",
    "Extracting \"%1$s\" to \"%2$s\" succeeded." : "La extracción de \"%1$s\" a \"%2$s\" se completó exitósamente.",
    "\"%s\" is not mounted." : "\"%s\" no está montado.",
    "Directory \"%s\" should be a mount point, but it is not." : "El directorio \"%s\" debería ser un punto de montaje, pero no lo es.",
    "Unknown personal setting: \"%1$s\"" : "Ajuste personal desconocido: \"%1$s\"",
    "Archive Explorer" : "Explorador de archivos",
    "bytes" : "bytes",
    "Archive Manager" : "Gestor de Archivos",
    "Archive Extraction" : "Extracción de Archivos",
    "Security Options" : "Opciones de seguridad",
    "Mount Options" : "Opciones de montaje",
    "archive format" : "formato de archivo",
    "unknown" : "desconocido",
    "MIME type" : "Tipo MIME",
    "uncompressed size" : "tamaño sin comprimir",
    "compressed size" : "tamaño comprimido",
    "not mounted" : "no montado",
    "Archive" : "Archivo",
    "reason unknown" : "razón desconocida",
    "true" : "cierto",
    "false" : "falso",
    "OK" : "OK",
    "Created" : "Creado",
    "Accepted" : "aceptado",
    "No Content" : "No hay contenido",
    "Reset Content" : "Reiniciar contenido",
    "Partial Content" : "Contenido parcial",
    "Moved Permanently" : "Movido permanentemente",
    "Found" : "Encontrado",
    "Use Proxy" : "Usar Proxy",
    "Unauthorized" : "No autorizado",
    "Forbidden" : "Prohibido",
    "Not Found" : "No encontrado",
    "Conflict" : "Conflicto",
    "Upgrade Required" : "Se necesita actualizar",
    "Too Many Requests" : "Demasiadas peticiones",
    "Unavailable For Legal Reasons" : "No disponible por razones legales",
    "Internal Server Error" : "Error interno del servidor",
    "Not Implemented" : "No está implementado",
    "Service Unavailable" : "Servicio no disponible",
    "Aborted" : "Cancelado",
    "Error" : "Error",
    "System Administrator" : "Administrador de Sistema",
    "Something went wrong." : "Algo ha ido mal.",
    "Internal Error" : "Error interno",
    "Missing data" : "Datos ausentes",
    "No" : "No",
    "Yes" : "Sí",
    "Debug Information" : "Información de depuración",
    "unknown error" : "error desconocido"
},
"nplurals=3; plural=n == 1 ? 0 : n != 0 && n % 1000000 == 0 ? 1 : 2;");
