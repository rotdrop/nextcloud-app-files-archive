OC.L10N.register(
    ...
    "Your archive file {source} has been extracted to {destination}. The archive contents reside there as ordinary files. Please feel free to use those as it pleases you. There is not automatic cleanup, the extracted files will remain in your file-space until you delete them manually. Just delete the folder {destination} if you have no more use of those files. Please note also that any changes committed to the extracted files will not be written back to the archive file {source }." : "Ihre Archivdatei {source} wurde nach {destination} extrahiert. Die Archivinhalte liegen dort als gewöhnliche Dateien vor. Bitte zögern Sie nicht, diese nach Belieben zu verwenden. Es erfolgt keine automatische Bereinigung. Die extrahierten Dateien bleiben in Ihrem Dateibereich, bis Sie sie manuell löschen. Löschen Sie einfach den Ordner {destination}, wenn Sie diese Dateien nicht mehr benötigen. Bitte beachten Sie auch, dass an den extrahierten Dateien vorgenommene Änderungen nicht in die Archivdatei {Quelle} zurückgeschrieben werden.",
