...
class ArchiveStorage extends AbstractStorage
{
   ...
   * does not exist any more, e.g. when the file-cache of the storage has been
   * lost. The cache lookups would then fail for every path including the
   * mount root, and the mount would look broken while nothing is able to
   * repair it: everything which could rebuild the root entry asks the very
   * same cache first. So check the root entry once and fall back to the lazy
   * archive scan if it is not there.
   *
   * @return bool
   */
